...
if (move_uploaded_file($_FILES['file']['tmp_name'], $uploadpath)) { //перемещение загруженного файла из временной папки сервера в папку, которую указали (uploadpath)
	$result = mysqli_query($link,"UPDATE `programs1` SET `name` = '$name', `Announcement` = '$Announcement', `Audience_category` = '$Audience_category', `Equipment` = '$Equipment', `Hours` = '$Hours', `form_of_training` = '$Form_of_training', `path` = '$uploadpath' WHERE `programs1`.`program_id` = $program_id"); //составляем запрос на запись в базу имя и путь к файлу
